...
2. 前端負責 enum 到多國語言字串的轉換，例如 `itemCategory = 1` 顯示為「原料」。後端可回傳 `categoryName` 作為繁中 fallback，但前端不得依賴此欄位作為唯一顯示來源。
3. 類別層級的庫存量因可能混合不同單位，Warehouse 第一版畫面不顯示庫存單位；後端若回傳 `unit`，混合單位時建議回傳 `0`。
4. 明細層級、風險警示與任務列仍保留 `unit` 欄位，因這些資料通常可對應單一料品與單位。
5. 風險說明文字與建議處理方式建議後端回傳 code 與繁中 fallback；正式多語顯示由前端依 code 與參數轉換。
