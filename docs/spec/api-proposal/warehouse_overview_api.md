...
| payload.inventoryValueByCategory[].categoryName | String | 料品品項類別顯示名稱。 |  | 建議由前端進行Enum轉換多國語言字串 |
...
| payload.riskAlerts[].message | String | 風險說明文字。 | 前端可直接顯示 | 尚未規劃與實作 |
| payload.riskAlerts[].recommendedAction | String | 建議處理方式。 | 前端可直接顯示 | 尚未規劃與實作 |
...
| payload.valueTrend[].categoryName | String | 料品品項類別顯示名稱。 |  | 建議由前端進行Enum轉換多國語言字串  |
...
| 項目 | 建議規則 | 狀態 | 工程師回覆 |
| --- | --- | --- | --- |
| 目前庫存數量 | 以 `inventory_record` 入庫數量減出庫數量，依倉儲、料品、批號分組。 | 基本可行，待工程師確認效能與邊界條件 | 建議採用每月的庫存數量與庫存價值(`inventory_month_statistic` 或 `inventory_item_month_statistic`) 搭配 每日庫存異動彙總 `inventory_delta`進行目前庫存數量計算。
...
| 安全水位 | 以目前數量或可用數量低於安全水位判斷。 | 待確認來源欄位 | 尚未規劃與實作 |
| 預留數量 | 需扣除訂單、工單備料與待處理倉庫任務。 | 待確認 | 尚未規劃與實作 |
| 品檢保留量 | 需扣除品保未放行或隔離量。 | 待確認 | 尚未規劃與實作 |
| 佔用板數 | 候選使用 `batchno_serialno_group.group` distinct count。 | 待確認 | 尚未規劃與實作 |
| 倉儲總板位 | 目前 DB 文件未找到明確欄位。 | 待確認 | 尚未規劃與實作 |
| 今日待處理 | 建議包含今日到期與逾期未完成任務。 | 待確認 | 同意 |
...
| payload.results[].currentQuantity | Float | 目前庫存數量。 | 入庫 - 出庫 |  | 建議採用每月的庫存數量與庫存價值(`inventory_month_statistic` 或 `inventory_item_month_statistic`) 搭配 每日庫存異動彙總 `inventory_delta`進行目前庫存數量計算。|
...
| 問題 | 影響 | 狀態 | 工程師回覆 | 
| --- | --- | --- | --- |
| 倉庫經營總覽是否建立新的 `/api/v1/warehouse/*` route，或擴充現有 `/api/v1/inventory/*`、`/api/v1/shipwarehouse/*`？ | 影響 API group 與 route ownership。 | 待確認 | 建立新的route, 並將版本升級為v2 `/api/v2/warehouse/*` |
| 庫存價值以 `inventory_record.amount`、統計表，還是 `item_price` 重新計算為準？ | 影響財務數字一致性。 | 待確認 | 庫存價值以 `inventory_record.amount` |
| 預留數量的正式來源是訂單、工單、倉庫任務中的哪些資料？ | 影響可用庫存。 | 待確認 | | 尚未規劃與實作 |
| 安全水位來源 table/field 是哪一個？ | 影響低於安全水位警示。 | 待確認 | | 尚未規劃與實作 |
| 品檢放行/保留狀態來源是哪些 table/field？ | 影響可用量與品檢保留量。 | 待確認 | | 尚未規劃與實作 |
| 倉儲總板位來源是哪些 table/field？ | 影響容量與可用板位。 | 待確認 | | 尚未規劃與實作 |
| `batchno_serialno_group.group` 是否可作為佔用板數計算依據？ | 影響板數統計。 | 待確認 | | 尚未規劃與實作 |
| 物料、膠捲是否排除於少於三分之一效期警示？ | 使用者已提出排除方向，需後端規則確認。 | 待確認 | | 物料、膠捲排除 |
| Dashboard 是否預設包含庫存明細列？ | 影響 response size 與前端 mapper。 | 待確認 | | 預設不包含庫存明細列 |
| 今日待處理是否包含逾期未完成任務？ | 影響管理者看到的風險與任務數量。 | 待確認 | | 包含 |
