...
class CInventoryDelta(IInventoryStatistic):
    ...
    @abstractmethod
    def _get_data(self, obj_session, dict_day):
        """必須由繼承類別實作的抽象方法"""
        pass
        ...
        if dict_with[0]:
            ...
            if lst_tmp:
                for obj_row in lst_tmp:
                    if obj_row.specified_no== "PMA0001001":
                        print("")
                    dict_data = {
    ...
    def __cal(self, obj_session, dict_day, f_refresh=False):
        ...
        if f_refresh or not lst_tmp:
            lst_result = self._get_data(obj_session, dict_day)
    ...
    def _get_data(self, obj_session, dict_day):
        ...
        lst_result = (
            obj_session.query(
                ...
                func.group_concat(
                    case((CTableInventoryRec.category == EInventoryCategory.IN, CTableInventoryRec.id),
                         else_=None)
                ).label("in_ids"),
...
class CInventoryItemMonth(IInventoryStatistic):
            ...
            with CDBMgr() as obj_dbmgr:
                ...
                for dict_data in lst_data:
                    ...
                    if dict_map and dict_data['specified_no'] in dict_map:
                        ...
                        dict_data['price'] = dict_map[dict_data['specified_no']]["price"] * 1.05
    ...
    def __cal(self, obj_session, str_month, n_specified_end=0, f_refresh=False):
        ...
        if self.__m_n_type == self.TYPE_ITEMS:
            lst_cal = CInventoryDeltaItem(self.m_str_timezone).retrieve_items_total(n_start, n_end)
        else:
            obj_invBatchNo = CInventoryDeltaBatchNo(self.m_str_timezone)
            if self.__m_str_item_no:
                obj_invBatchNo.setParam(self.__m_str_item_no)
            lst_cal = obj_invBatchNo.retrieve_items_total(n_start, n_end)
        #lst_cal = []
        lst_pre = [] # 含稅
    ...
    def __merge_lists(self, str_date, lst_a, lst_b):
        ...
        for a_item in lst_a:
            ...
            if a_item["specified_no"] == "PMA0001001":
                print("")
            if b_item:
                ...
                f_amount = ((b_item.get("inAmount", 0.0) - b_item.get("outAmount", 0.0)) * 1.05)
        ...
        for b_item in lst_b:
            ...
            if key not in matched_keys:
                new_item = {
                    ...
                    "endAmount": round((b_item.get("inAmount", 0.0) - b_item.get("outAmount", 0.0)) * 1.05 ,2)
                }
    ...
    def __fill_query_params(self):
        if self.__m_n_type == self.TYPE_BATCHNO:
            lst_where = [CTableInventoryItemMonthStatistic.kind== EInventoryDeltaKind.BATCHNO]
            if self.__m_str_item_no:
                lst_where.append(CTableInventoryItemMonthStatistic.specified_ref_no == self.__m_str_item_no)
        if self.__m_n_type == self.TYPE_ITEMS:
            lst_where = [CTableInventoryItemMonthStatistic.kind != EInventoryDeltaKind.BATCHNO]
            if self.__m_str_item_no:
                lst_where.append(CTableInventoryItemMonthStatistic.specified_no == self.__m_str_item_no)
        return lst_where
        ...
        for n_idx1 in range(EInventoryDeltaKind.MATERIAL, EInventoryDeltaKind.PRODUCT+1):
            for n_idx2 in range(0, 4):
                ...
                if lst_filter:
                    ...
                    f_amount = round((obj_data.total_inAmount - obj_data.total_outAmount) * 1.05, 2)
                    print(n_category)
                    ...
                    dict_data["inPurchaseAmount"] = round(obj_data.total_inPurchaseAmount * 1.05)
                    dict_data["inAmount"] = round(obj_data.total_inAmount* 1.05)
                    dict_data["outAmount"] = round(obj_data.total_outAmount * 1.05)
