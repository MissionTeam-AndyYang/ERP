...
| payload.results[].companyName | String | 客戶/廠商名稱 |  |
...
2. 查詢 company、order_payment、payment、shipping_order 取得銷售 / 應收應付資料
3. 計算符合條件的總筆數與本次回傳筆數
4. 整理查詢結果清單並展開回傳欄位語意
...
2. 查詢 contract、order_payment、product_order、shipping_order 取得銷售 / 帳款資料
3. 計算符合條件的總筆數與本次回傳筆數
4. 整理查詢結果清單並展開回傳欄位語意
...
| payload.results[].itemCategory | Integer | 「交易品項」類別 |  |
| payload.results[].itemSubCategory | Integer | 「交易品項」子類別 |  |
| payload.results[].realCount | Integer | 銷貨數量 |  |
| payload.results[].realAmount | Integer | 銷貨金額 |  |
...
2. 查詢 contract、goods_receipt_note、material、product_order 取得銷售 / 訂購單資料
3. 計算符合條件的總筆數與本次回傳筆數
4. 整理查詢結果清單並展開回傳欄位語意
...
| payload.results[].no | String | 資料編號 |  |
| payload.results[].creator_no | String | 製單人員編號 |  |
| payload.results[].purchase_order_no | String | 訂購單號 |  |
| payload.results[].date | Integer | 日期時間 |  |
...
1. 讀取查詢條件並轉換為業務篩選條件：count、start
2. 查詢 batch_number、inventory_record、product_order、shipping_order 取得銷售 / 銷貨單資料
3. 計算符合條件的總筆數與本次回傳筆數
4. 整理查詢結果清單並展開回傳欄位語意
