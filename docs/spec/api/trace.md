...
| payload.traceSteps[].inputItems[].quantity | Float | 此步驟實際投入數量，取至小數點第 2 位；以 `production_data_input.action=1` 領料數量加總扣除 `action=2` 退料數量加總後回傳 |  |
