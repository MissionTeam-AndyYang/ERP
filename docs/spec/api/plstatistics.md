...
| Table | Purpose |
|----------|------|
| pl_item_capacity | 程式碼路徑中透過 ORM/服務邏輯使用此資料表 |
...
1. 讀取查詢條件：count、start
2. 呼叫服務：CLogger.log
3. 查詢或使用資料表：pl_item_capacity、pl_item_loss
4. 組裝回傳 payload 欄位：payload.total、payload.count、payload.results
...
|----------|------|
| pl_item_capacity | 程式碼路徑中透過 ORM/服務邏輯使用此資料表 |
| pl_item_loss | 程式碼路徑中透過 ORM/服務邏輯使用此資料表 |
...
| Table | Purpose |
|----------|------|
| pl_man_capacity | 程式碼路徑中透過 ORM/服務邏輯使用此資料表 |
