...
| Priority | Phase | Code | Type | 正式畫面名稱 | Route / UI Location | Implementation Status | Primary API | 說明 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | V1 Core | `PurchasingWorkspaceScreen` | Screen | 採購與備料工作區 | `/purchasing` | 已有第一版，已串接 PO-first dashboard、delivery-risk、goods-receipts、suppliers 與 selected PO detail API | `GET /api/v2/purchasing/purchase-orders/dashboard`、`GET /api/v2/purchasing/purchase-orders/delivery-risk`、`GET /api/v2/purchasing/goods-receipts/dashboard`、`GET /api/v2/purchasing/suppliers/dashboard`、`GET /api/v2/purchasing/purchase-orders/{purchase_order_no}/detail` | 採購中心入口，以採購單為主資料列，呈現採購 KPI、交期風險、到貨驗收與供應商追蹤；API 錯誤時顯示錯誤，不自動改用 mock。 |
| P0.1 | V1 Core | `PurchasingPurchaseOrderView` | View | 採購單視圖 | `PurchasingWorkspaceScreen` 內的「採購單」頁籤 | 已有第一版，已串接 dashboard API | `GET /api/v2/purchasing/purchase-orders/dashboard` | 支援任意歷史區間查詢，以採購單追蹤供應商、數量、金額、預計到貨與請購關聯。 |
| P0.2 | V1 Core | `PurchasingDeliveryRiskView` | View | 交期風險視圖 | `PurchasingWorkspaceScreen` 內的「交期風險」頁籤 | 已有第一版，已串接 delivery-risk API | `GET /api/v2/purchasing/purchase-orders/delivery-risk` | 聚焦逾期／今日到貨、未收缺口、正式來源影響與 workflow 阻塞。 |
| P0.3 | V1 Core | `PurchasingReceivingView` | View | 到貨驗收入庫視圖 | `PurchasingWorkspaceScreen` 內的「到貨驗收入庫」頁籤 | 已有第一版，已串接 goods-receipts dashboard API | `GET /api/v2/purchasing/goods-receipts/dashboard` | 以進貨單為主呈現分批到貨、退回、實際收貨數量與入庫交接；V1 不顯示品檢欄位。 |
| P0.4 | V1 Core | `PurchasingSupplierView` | View | 供應商追蹤視圖 | `PurchasingWorkspaceScreen` 內的「供應商追蹤」頁籤 | 已有第一版，已串接 suppliers dashboard API | `GET /api/v2/purchasing/suppliers/dashboard` | 依供應商彙總採購單、逾期、未收數量與採購金額。 |
| P1 | V1 Core | `PurchasingPurchaseOrderDetailPanel` | Panel | 採購單追蹤面板 | `PurchasingWorkspaceScreen` 右側 panel；窄版可作 drawer 或 detail route | 已有第一版，已串接 selected purchase order detail API | `GET /api/v2/purchasing/purchase-orders/{purchase_order_no}/detail` | 顯示採購單、請購、進貨、供應商、庫存與 workflow 關聯；detail API 失敗時只保留清單摘要並顯示錯誤。 |
...
| Priority | Phase | Code | Type | 正式畫面名稱 | Route / UI Location | Implementation Status | Primary API | 說明 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | V1 Core | `RecipeFormulaScreen` | Screen | Recipe / Formula | `/recipe` | 前端已完成 read-only UX、API client 與 mock/API 分離；已對齊 Backend `ERP2-RECIPE-FORMULA-RO-EXEC-001-BE-001` 正式欄位；DB-backed acceptance retest PASS，截圖自動化受本機工具限制 | `GET /api/v2/recipe-formula/dashboard`、`GET /api/v2/recipe-formula/{recipe_no}/versions/{version}/composition` | 檢視 Recipe 定義、目前版本、Formula 組成、唯一定義產出、來源 lineage、警示、Product Structure 參照與 Routing context 參照；不包含寫入或核准操作。 |
...
| Priority | Phase | Code | Type | 正式畫面名稱 | Route / UI Location | Implementation Status | Primary API | 說明 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | V1 Core | `RoutingProcessFlowScreen` | Screen | Routing / Process Flow | `/routing` | 前端已完成 read-only UX、API client 與 mock/API 分離；已對齊 Backend `ERP2-ROUTING-PROCESS-FLOW-RO-EXEC-001-BE-001` 正式欄位；Shared DEV real-backend validation 待 routing source table fixture | `GET /api/v2/routing/dashboard`、`GET /api/v2/routing/products/{item_no}/versions`、`GET /api/v2/routing/versions/{routing_version_id}/steps`、`GET /api/v2/routing/products/{item_no}/current` | 檢視 Product / WIP 適用 Routing Version、ordered process flow、Recipe reference、Packaging context、resource eligibility、standard performance、source lineage 與 controlled warnings；不包含寫入或執行操作。 |
| P0.1 | V1 Core | `RoutingProductWipSelectorView` | View | Product / WIP 選擇視圖 | `RoutingProcessFlowScreen` 左側清單 | 前端已完成 | `GET /api/v2/routing/dashboard` | 支援 Product / WIP / Routing 關鍵字查詢，顯示品項、Routing no、版本、步驟數、警示數與來源。 |
| P0.2 | V1 Core | `RoutingVersionSummaryPanel` | Panel | Routing 版本摘要面板 | `RoutingProcessFlowScreen` 右側 panel | 前端已完成 | `GET /api/v2/routing/products/{item_no}/versions`、`GET /api/v2/routing/versions/{routing_version_id}/steps` | 顯示選取 Product / WIP 的 Routing Version、版本狀態、流程步驟、Recipe reference 與警示摘要。 |
| P0.3 | V1 Core | `OrderedProcessFlowView` | View | 製程流程順序視圖 | `RoutingProcessFlowScreen` 主內容區 | 前端已完成 | `GET /api/v2/routing/versions/{routing_version_id}/steps` | 依 `process_flow.order` 顯示 step、stage、group、process identity、process label、標準產出、標準時間、效率基準與資源資格。 |
| P0.4 | V1 Core | `RoutingContextReferenceView` | View | Recipe 與 Packaging 參照視圖 | `RoutingProcessFlowScreen` 主內容區 | 前端已完成 | `GET /api/v2/routing/versions/{routing_version_id}/steps` | 顯示已建立 Recipe reference 與 bounded Packaging context；只作參照，不實作 Recipe、Packaging Specification 或 Manufacturing Definition。 |
| P0.5 | V1 Core | `RoutingGovernanceReferenceView` | View | 資源與標準表現視圖 | `RoutingProcessFlowScreen` 主內容區 | 前端已完成 | `GET /api/v2/routing/versions/{routing_version_id}/steps` | 顯示受治理 resource eligibility 與 standard performance reference；不進行排程、產能配置或派工。 |
| P0.6 | V1 Core | `RoutingLineageWarningPanel` | Panel | Routing 來源與警示面板 | `RoutingProcessFlowScreen` 主內容區 | 前端已完成 | `GET /api/v2/routing/versions/{routing_version_id}/steps` | 顯示 source lineage、缺少流程、Recipe reference、Packaging context、resource eligibility 或 standard performance 的 controlled warnings。 |
...
`BatchCenterScreen` 對應目前 `/batches` 執行畫面，第一版以料品與批號為主視角，提供批號分布、可用性、品檢保留、效期風險與關聯任務的唯讀檢視。它不取代 `WarehouseInventoryLotListScreen` 的倉庫作業視角，也不取代 `TraceabilityWorkspaceScreen` 的批號追溯視角。
...
批號中心只處理「目前仍有庫存的批號管理與可用性判讀」。若需要完整來源、製程投入、追溯鏈或批號異動流水帳，應使用或另行規劃 `TraceabilityWorkspaceScreen`、`WarehouseInventoryMovementLedgerScreen` 或其他下一版畫面；召回與文件完整性留待下一版規劃。

## Traceability Workspace Screen Roadmap

`TraceabilityWorkspaceScreen` 對應目前 `/traceability` 執行畫面，第一版以批號為主要查詢入口，提供來源、入庫、庫存、生產投入、產出、品檢與 workflow 節點的 read-only 追溯檢視。它承接 `BatchCenterScreen` 的批號資訊，但不處理批號庫存管理、品檢保留管理、出入庫任務執行、召回評估或文件完整性。
...
第一版仍以 read-only 為主，不建立、修改或停用公司主檔、料品主檔或交易品項。`MaterialItemMasterView` 延續既有 `ItemCenterScreen` 的料品主資料範圍；`CompanyMasterView` 與 `TransactionItemMasterView` 補齊由 `company`、`trans_items`、`contract` 形成的商務主資料視角。交易品項必須顯示所屬客戶／廠商與對應料品，避免僅以料號或合約資訊孤立呈現；`trans_items2` 留待下一版資料來源規劃後再納入。

| Priority | Phase | Code | Type | 正式畫面名稱 | Route / UI Location | Implementation Status | Primary API | 說明 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | V1 Core | `ItemAndTransactionMasterScreen` | Screen | 品項與交易主資料中心 | `/items` | 後端 API 已實作，待工程師 runtime review | `GET /api/v2/items/dashboard`、`GET /api/v2/transitems/dashboard` | 同一入口整合客戶／廠商、料品品項與交易品項三個 read-only 視角，提供主資料查詢、關聯檢查與資料缺口辨識。 |
| P0.1 | V1 Core | `CompanyMasterView` | View | 客戶／廠商主檔視圖 | `ItemAndTransactionMasterScreen` 內的「客戶／廠商」頁籤 | 後端 API 已實作，待工程師 runtime review | `GET /api/v2/transitems/dashboard`、`GET /api/v2/transitems/companies/{company_no}/detail` | 顯示 `company` 主檔、交易角色、聯絡資訊、付款/收款基礎資訊，以及與交易品項、合約、採購/訂購流程的摘要關聯。 |
| P0.2 | V1 Core | `MaterialItemMasterView` | View | 料品品項視圖 | `ItemAndTransactionMasterScreen` 內的「料品品項」頁籤 | 後端 API 已實作，待工程師 runtime review | `GET /api/v2/items/dashboard`、`GET /api/v2/items/{item_no}/detail` | 延續既有料品主資料範圍，檢視原料、物料、膠捲、在製品、製成品與貨品主檔，包含主檔完整度、庫存訊號、BOM 關聯與維護風險。 |
| P0.3 | V1 Core | `TransactionItemMasterView` | View | 交易品項視圖 | `ItemAndTransactionMasterScreen` 內的「交易品項」頁籤 | 後端 API 已實作，待工程師 runtime review | `GET /api/v2/transitems/dashboard`、`GET /api/v2/transitems/transitems/{transaction_item_no}/detail` | 顯示由客戶／廠商合約或交易條件產生的交易品項，包含交易品項、所屬公司、對應料品、交易單位、交易單價、物流成本與合約來源。 |
| P0.4 | V1 Core | `ItemCategorySummaryView` | View | 品項分類摘要視圖 | `MaterialItemMasterView` 內的分類摘要與 KPI | 後端 API 已實作，待工程師 runtime review | `GET /api/v2/items/dashboard` | 依料品品項類別彙總品項數、目前有庫存品項數、BOM 關聯數與需注意品項數。 |
| P0.5 | V1 Core | `ItemMasterListView` | View | 料品主檔清單視圖 | `MaterialItemMasterView` 主要清單 | 後端 API 已實作，待工程師 runtime review | `GET /api/v2/items/dashboard` | 支援關鍵字、類別、主檔狀態、是否有庫存、是否有 BOM 關聯與分頁。 |
| P0.6 | V1 Core | `ItemMaintenanceSuggestionView` | View | 主檔維護建議視圖 | `MaterialItemMasterView` 右側待維護面板 | 後端 API 已實作，待工程師 runtime review | `GET /api/v2/items/dashboard` | 顯示缺單位、缺 BOM、缺庫存訊號等 read-only 維護建議；不代表 workflow task 或部門轉交。 |
| P1 | V1 Core | `CompanyDetailPanel` | Panel | 客戶／廠商明細面板 | `CompanyMasterView` 右側 panel；窄版可作 drawer | 後端 API 已實作，待工程師 runtime review | `GET /api/v2/transitems/companies/{company_no}/detail` | 顯示單一公司主檔、交易角色、聯絡與帳款摘要、交易品項清單及近期合約摘要。 |
| P1.1 | V1 Core | `ItemDetailPanel` | Panel | 料品明細面板 | `MaterialItemMasterView` 右側 panel；窄版可作 drawer | 後端 API 已實作，待工程師 runtime review | `GET /api/v2/items/{item_no}/detail` | 顯示單一料品主檔、庫存摘要、BOM 使用、近期批號與主檔維護建議。 |
| P1.2 | V1 Core | `TransactionItemDetailPanel` | Panel | 交易品項明細面板 | `TransactionItemMasterView` 右側 panel；窄版可作 drawer | 後端 API 已實作，待工程師 runtime review | `GET /api/v2/transitems/transitems/{transaction_item_no}/detail` | 顯示單一交易品項的公司、對應料品、交易單位、交易單價、物流成本、規格與合約來源。 |
...
| Priority | Phase | Code | Type | 正式畫面名稱 | Route / UI Location | Implementation Status | Primary API | 說明 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CP1-P0 | CP1 Proposal | `ProductWip360OverviewScreen` | Screen | Product / WIP 360 唯讀總覽 | 建議 route：`/product-360`；可由 `/items`、`/bom`、`/recipe`、`/routing`、`/batches` drill-down 進入 | UX / Static Preview Proposal；Pending Engineer Review；尚未授權 runtime frontend implementation | Proposed: `GET /api/v2/product-wip-360/overview` | 以 `itemNo + itemCategory` 查詢單一製成品或 standalone WIP，呈現主檔、交易品項、庫存、批號、BOM、Recipe、Routing、source lineage、warnings 與 module readiness。 |
| CP1-P0.1 | CP1 Proposal | `ProductWipSummarySection` | View | Product / WIP 摘要區 | `ProductWip360OverviewScreen` 內的上方摘要 | UX / Static Preview Proposal | Proposed overview payload | 顯示 Product/WIP 主檔、類別、單位、版本、交易品項、客戶／廠商參照與 identity mismatch warning。 |
| CP1-P0.2 | CP1 Proposal | `ProductWipInventoryQuantitySection` | View | 庫存與數量狀態區 | `ProductWip360OverviewScreen` 內的庫存與批號區 | UX / Static Preview Proposal | Proposed overview payload，來源參照既有 Warehouse / Batch API | 顯示目前庫存、可用量、預留、品檢保留、庫存價值、倉庫數、批號數與風險批號摘要。 |
| CP1-P0.3 | CP1 Proposal | `ProductWipStructureBomSection` | View | 產品結構與 BOM 區 | `ProductWip360OverviewScreen` 內的 BOM 區 | UX / Static Preview Proposal | Proposed overview payload，來源參照既有 BOM API | 顯示 current BOM、版本狀態與產品結構摘要；不處理 BOM 編輯、核准或發布。 |
| CP1-P0.4 | CP1 Proposal | `ProductWipRecipeFormulaSection` | View | Recipe / Formula 區 | `ProductWip360OverviewScreen` 內的 Recipe 區 | UX / Static Preview Proposal | Proposed overview payload，來源參照既有 Recipe API | 顯示 Recipe no、版本、Formula 組成摘要與 output 定義；不處理 Recipe 編輯、核准或發布。 |
| CP1-P0.5 | CP1 Proposal | `ProductWipRoutingProcessSection` | View | Routing / Process Flow 區 | `ProductWip360OverviewScreen` 內的 Routing 區 | UX / Static Preview Proposal | Proposed overview payload，來源參照既有 Routing API | 顯示 Routing version、ordered process flow、Recipe reference、resource eligibility 與 standard performance 狀態；不處理排程、派工或製造執行。 |
| CP1-P0.6 | CP1 Proposal | `ProductWipSourceWarningPanel` | Panel | 來源與警示面板 | `ProductWip360OverviewScreen` 右側 panel | UX / Static Preview Proposal | Proposed overview payload | 彙整各模組 complete / partial / unavailable / test_support / error 狀態，並顯示 test-support、partial、unresolved、unavailable、not-applicable 與 mismatch warnings；API mode 不得以 mock 取代空陣列或錯誤狀態。 |
...
| `TraceabilityWorkspaceScreen` | 已有第一版，已串接 `GET /api/v2/trace/dashboard` 與 `GET /api/v2/trace/batches/{batch_no}/overview`；第一版暫不納入召回評估與文件完整性。 | 依工程師實機資料檢查 dashboard payload、selected batch overview payload、traceSteps 顯示、空狀態與 API/Mock 切換邊界。 |
