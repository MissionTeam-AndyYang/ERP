...
| Priority | Phase | Code | Type | 正式畫面名稱 | Route / UI Location | Implementation Status | Primary API | 說明 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CP1-P0 | CP1 Implementation | `ProductWip360OverviewScreen` | Screen | Product / WIP 360 唯讀總覽 | `/product-360`；可由 `/items`、`/bom`、`/recipe`、`/routing`、`/batches` drill-down 進入 | 前端已完成 bounded non-production read-only 第一版；待後端 API implementation / runtime review | Proposed: `GET /api/v2/product-wip-360/overview` | 以 `itemNo + itemCategory` 查詢單一製成品或 standalone WIP，呈現主檔、交易品項、庫存、批號、BOM、Recipe、Routing、source lineage、warnings 與 module readiness。 |
