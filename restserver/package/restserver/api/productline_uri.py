...
@productline.route(URL_PATH+'/'+SUBKEY+'/process', methods=['GET'])
def process():
    obj_uri = CProcessURI()
    return obj_uri.run()

@productline.route(URL_PATH+'/'+SUBKEY, methods=['GET'])
def index():
    obj_uri = CProductLineURI()
    return obj_uri.run()


@productline.route(URL_PATH+'/'+SUBKEY+'/station', methods=['GET'])
def station():
    obj_uri = CStationURI()
    return obj_uri.run()

@productline.route(URL_PATH+'/'+SUBKEY+'/equipment', methods=['GET'])
def equipment():
    obj_uri = CEquipmentURI()
    return obj_uri.run()

@productline.route(URL_PATH+'/'+SUBKEY+'/factory', methods=['GET'])
def factory():
    obj_uri = CFactoryURI()
    return obj_uri.run()
