...
from package.common.common import EGoodsReceiptNoteCategory, EWorkflowTaskStatus, EWorkflowTaskType
from package.dbwrapper.dbmgr import CDBMgr
from package.dbwrapper.table import (
    CTableCompany, CTableGoodsReceiptNote, CTableInventoryRec, CTablePurchaseOrder,
    CTablePurchaseRequest, CTableWorkflowTaskState,
)
...
class CPurchasingPurchaseOrderService(object):
    ...
    def __row(self, obj_session, obj_order, dict_receipts, n_now):
        ...
        str_risk = "normal"
        str_risk_code = "normal"
        if f_open > 0:
            if n_expected and n_expected < n_now:
                str_risk, str_risk_code = "high_risk", "late_arrival"
                ...
                str_risk, str_risk_code = "notice", "due_today"
            elif not obj_request:
                str_risk, str_risk_code = "notice", "purchase_request_unlinked"
            else:
                str_risk, str_risk_code = "notice", "open_receipt"
        return {
            ...
            "riskLevel": str_risk, "riskCode": str_risk_code,
        }
    ...
    def get_dashboard(self):
        ...
        with CDBMgr() as obj_dbmgr:
            ...
            return self.__response(dict_range, {"summary": {
                ...
                "lateOrDueTodayCount": sum(1 for dict_row in lst_rows if dict_row["riskCode"] in ("late_arrival", "due_today")),
    ...
    def get_delivery_risk(self):
        ...
        if dict_payload is None:
            return None
        lst_rows = [dict_row for dict_row in dict_payload["items"] if dict_row["riskLevel"] != "normal"]
        dict_payload["items"] = lst_rows
        dict_payload["total"] = len(lst_rows)
        dict_payload["count"] = len(lst_rows)
        dict_payload["summary"] = {
            "highRiskCount": sum(1 for dict_row in lst_rows if dict_row["riskLevel"] == "high_risk"),
            "noticeCount": sum(1 for dict_row in lst_rows if dict_row["riskLevel"] == "notice"),
            "lateCount": sum(1 for dict_row in lst_rows if dict_row["riskCode"] == "late_arrival"),
    ...
    def get_suppliers(self):
        ...
        for dict_row in dict_payload["items"]:
            ...
            dict_item = dict_rows.setdefault(str_key, {"supplierNo": str_key, "supplierName": dict_row["supplierName"],
                ...
                "purchaseAmount": 0, "pendingReceiptCount": 0, "riskLevel": "normal"})
            dict_item["purchaseOrderCount"] += 1
            dict_item["openPurchaseOrderCount"] += int(dict_row["openCount"] > 0)
            dict_item["latePurchaseOrderCount"] += int(dict_row["riskCode"] == "late_arrival")
            dict_item["purchaseAmount"] += dict_row["purchaseAmount"]
            dict_item["pendingReceiptCount"] += dict_row["openCount"]
            if dict_row["riskLevel"] == "high_risk" or (dict_row["riskLevel"] == "notice" and dict_item["riskLevel"] == "normal"):
...
class CPurchasingPurchaseOrder(object):
    def get(self, str_timezone, str_id):
        obj_service = CPurchasingPurchaseOrderService(str_timezone)
        if request.path.endswith("/delivery-risk"):
            dict_payload = obj_service.get_delivery_risk()
        elif request.path.endswith("/goods-receipts/dashboard"):
            dict_payload = obj_service.get_receipts()
        elif request.path.endswith("/suppliers/dashboard"):
            dict_payload = obj_service.get_suppliers()
        elif str_id:
            dict_payload = obj_service.get_detail(str_id)
        else:
