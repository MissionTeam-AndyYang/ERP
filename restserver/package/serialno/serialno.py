...
from package.dbwrapper.dbmgr import CDBMgr, CDBMgrTrans
import uuid
...
class CBSNoSubject(object):
    ...
    def add(self, lst_batchNo, str_hardwareId=""):
        n_code =  EErrorCode.ERROR_SUCCESS
        try:
            with CDBMgr() as obj_dbmgr:
                for dict_data in lst_batchNo:
                    if dict_data.get("batchno", ""):
                        ...
                        if obj_batch:
                            ...
                            # 拆解子表單
                            dict_tmp["ref_no_sub"] = self.__convent_ref_no_sub(obj_batch,  dict_tmp["ref_no_sub"], n_refno_subCategory)
                        # 取得品項編號
                            ...
                            for dict_serialNo in lst_data:
                                ...
                                if dict_serialNo["isValid"]:
                                    ...
                                    if n_BSType in [EBSType.PURCHASE_IN_S, EBSType.PRODUCT_RETURN_IN_S, EBSType.PRODUCT_IN_S, EBSType.SALE_IN_S, EBSType.OTHER_IN_S]:
                                        # 入庫
                                        # 1.新增入庫紀錄
                                        self.__add_update_inventory(obj_dbmgr, obj_batch, EInventoryCategory.IN, dict_tmp, dict_serialNo, str_hardwareId)
                                        if n_BSType in [EBSType.PURCHASE_IN_S, EBSType.SALE_IN_S]:
                                            #2. 新增 批號+序號 進貨單
                                            self.__add_update_batchNo_serialNo(obj_dbmgr, obj_batch, dict_tmp, dict_serialNo)
                                    elif n_BSType in [EBSType.PURCHASE_OUT_S, EBSType.PRODUCT_OUT_S, EBSType.SALE_OUT_S, EBSType.OTHER_OUT_S]:
                                        #出庫
                                        # 1.新增出庫紀錄
                                        self.__add_update_inventory(obj_dbmgr, obj_batch, EInventoryCategory.OUT, dict_tmp, dict_serialNo, str_hardwareId)
                                    else:
                                        #產間
                                        # 1.新增製造數據
                                        ...
                                        if n_BSType == EBSType.PRODUCT_OUT_P:
                                            ...
                                            if n_refno_subCategory == EProcessOrderCategory.REMAIN:
                                                self.__add_update_productiondata_reuse(obj_dbmgr, str_data_id, dict_tmp, dict_serialNo)
                                            if n_refno_subCategory == EProcessOrderCategory.WASTE:
                                                self.__add_update_productiondata_reuse(obj_dbmgr, str_data_id, dict_tmp, dict_serialNo)
                                            if n_refno_subCategory == EProcessOrderCategory.PRODUCT:
                                                self.__add_update_productiondata_output(obj_dbmgr, str_data_id, dict_tmp, dict_serialNo)
                                        elif n_BSType in [EBSType.PRODUCT_IN_P, EBSType.PRODUCT_RETURN_OUT_P]:
                                            # 領/退料
                                            self.__add_update_productiondata_input(obj_dbmgr, str_data_id, dict_tmp, dict_serialNo)
                                else:
                                    pass #不相符的資料
                            # 通知註冊函式 , 更新批號/訂單檢定數量
                            dict_param = {
    ...
    def __add_update_inventory(self, obj_dbmgr, obj_batch, n_category, dict_data, dict_serialNo, str_hardwareId):
        obj_session = obj_dbmgr.get_session()
        str_warehouse_no = "WH4250218PDL"
        str_warehouse_displayName = self.__get_warehouse(obj_session, str_warehouse_no)
        ...
        if obj_batch:
            # 新增/更新資料
            lst_where = [
                CTableInventoryRec.ref_no == str_ref_no,
                CTableInventoryRec.category == n_category,
                CTableInventoryRec.batchNumber == str_batchno,
                CTableInventoryRec.serialNo == dict_serialNo["serialNo"],
                CTableInventoryRec.registerDevId == str_hardwareId
            ]
            ...
            if obj_result:
                # 更新
                dict_update = {"date": n_date,
                               "count": f_count,
                               "amount": round(f_count * f_price, 0),
                               "creationTime":util_retrieve_now_time()}
                if obj_dbmgr.update(CTableInventoryRec, lst_where,
                                    dict_update) != EErrorCode.ERROR_SUCCESS:
                    str_message = 'failed to update inventory'
                    CLogger().log(CLogger.LOG_LEVELERROR, '[%s] %s (ref_no:%s, batch_no:%s, serial_no:%s, device_id:%s)' % (
                        self.__class__.__name__, str_message, str_ref_no, str_batchno, dict_serialNo["serialNo"], str_hardwareId))
            else:
                dict_inventory = {
                    "creator_id": "",
                    "group": str_group,
                    ...
                    "warehouse_displayName": str_warehouse_displayName,
                    "date": n_date,
    ...
    def __add_update_batchNo_serialNo(self, obj_dbmgr, obj_batch, dict_data, dict_serialNo):
        str_warehouse_no = "WH4250218PDL"
        obj_session = obj_dbmgr.get_session()
        #n_date = dict_data["date"]
        n_date = dict_serialNo["devDateTimestamp"]
        ...
        # 關聯 進貨單/派工單
        lst_where = [
            CTableBatchNoSerialNo.serialNo == dict_serialNo["serialNo"],
            CTableBatchNoSerialNo.batch_number == str_batchno,
            CTableBatchNoSerialNo.ref_order_no == str_ref_no
        ]
        ...
        if obj_data:
            if obj_dbmgr.update(CTableBatchNoSerialNo, lst_where,
                                {"time": n_date,
                                 "count": dict_serialNo["value"],
                                 "updatedTime":util_retrieve_now_time()}) != EErrorCode.ERROR_SUCCESS:
                str_message = 'failed to update batchno_serialno'
                CLogger().log(CLogger.LOG_LEVELERROR, '[%s] %s' % (self.__class__.__name__, str_message))
                CLogger().log(CLogger.LOG_LEVELERROR, '[%s] %s (ref_no:%s, batch_no:%s, serial_no:%s)' % (
            ...
            new_data = CTableBatchNoSerialNo(
                ...
                ref_order_no= str_ref_no,#str_ref_no_sub if str_ref_no_sub else str_ref_no,
                ref_order_no_category = n_refno_category,#n_refno_subCategory if str_ref_no_sub else n_refno_category,
                time= n_date,
            ...
            if obj_dbmgr.insert(new_data) != EErrorCode.ERROR_SUCCESS:
                str_message = 'failed to insert batchno_serialno'
                CLogger().log(CLogger.LOG_LEVELERROR, '[%s] %s' % (self.__class__.__name__, str_message))
    ...
    def __add_update_productiondata_input(self, obj_dbmgr, str_data_id,  dict_data, dict_serialNo):
        # 領退料
        obj_session = obj_dbmgr.get_session()
        #n_date = dict_data["date"]
        n_date = dict_serialNo["devDateTimestamp"]
        ...
        if obj_item:
            n_action = EInputAction.RECEIVE if n_refno_subCategory == EProcessOrderCategory.RECEIVE else EInputAction.RETURN if n_refno_subCategory == EProcessOrderCategory.RETURN else 0
            lst_where = [
                CTableProductionDataInput.work_order_no == str_ref_no,
                CTableProductionDataInput.group == "",
                CTableProductionDataInput.action == n_action,
                CTableProductionDataInput.item_no == obj_item.item_no,
                CTableProductionDataInput.serial_no == dict_serialNo["serialNo"],
                CTableProductionDataInput.batch_number == str_batchno,
            ]
            obj_result = (
            ...
            if obj_result:
                if obj_dbmgr.update(CTableProductionDataInput, lst_where,
                                    {"time": n_date,
                                     "count": dict_serialNo["value"],
                                     "comment": str_comment}) != EErrorCode.ERROR_SUCCESS:
                    str_message = 'failed to update production data input'
                    CLogger().log(CLogger.LOG_LEVELERROR, '[%s] %s (ref_no:%s, batch_no:%s, serial_no:%s)' % (
                        self.__class__.__name__, str_message, str_ref_no, str_batchno, dict_serialNo["serialNo"]))
            else:
    ...
    def __add_update_productiondata_output(self, obj_dbmgr, str_data_id, dict_data, dict_serialNo):
        # 產品單
        obj_session = obj_dbmgr.get_session()
        #n_date = dict_data["date"]
        n_date = dict_serialNo["devDateTimestamp"]
        ...
        str_uuid = str(uuid.uuid4()).replace("-", "")
        str_comment = dict_data["comment"]
        n_group = self.__count_ref_no_sub(str_ref_no_sub)
        str_group = 'group_'+ str(n_group)
        obj_item = (
            obj_session.query(CTableProcessOrder)
            .filter(CTableProcessOrder.no == str_ref_no_sub)
            .first()
        ...
        if obj_item:
            lst_where = [
                CTableProductionDataOutput.work_order_no == str_ref_no,
                CTableProductionDataOutput.group == str_group,
                CTableProductionDataOutput.action ==  EOutputAction.WORK,
                CTableProductionDataOutput.item_no == obj_item.item_no,
                CTableProductionDataOutput.serial_no == dict_serialNo["serialNo"],
                CTableProductionDataOutput.batch_number == str_batchno,
            ]
            obj_result = (
            ...
            if obj_result:
                if obj_dbmgr.update(CTableProductionDataOutput, lst_where,
                                    {"time": n_date,
                                     "count": dict_serialNo["value"],
                                     "comment": str_comment}) != EErrorCode.ERROR_SUCCESS:
                    str_message = 'failed to update production data output'
                    CLogger().log(CLogger.LOG_LEVELERROR, '[%s] %s (ref_no:%s, batch_no:%s, serial_no:%s)' % (
                        self.__class__.__name__, str_message, str_ref_no, str_batchno, dict_serialNo["serialNo"]))
            else:
    ...
    def __add_update_productiondata_reuse(self, obj_dbmgr, str_data_id,  dict_data, dict_serialNo):
        # 餘/廢料單
        obj_session = obj_dbmgr.get_session()
        #n_date = dict_data["date"]
        n_date = dict_serialNo["devDateTimestamp"]
        ...
        if obj_item:
            n_category = EReuseCategory.REMAINING if n_refno_subCategory == EProcessOrderCategory.REMAIN else  EReuseCategory.WASTE if n_refno_subCategory == EProcessOrderCategory.WASTE else 0 # 餘/廢料
            lst_where = [
                CTableProductionDataReuse.work_order_no == str_ref_no,
                CTableProductionDataReuse.category == n_category,
                CTableProductionDataReuse.item_no == obj_item.item_no,
                CTableProductionDataReuse.serial_no == dict_serialNo["serialNo"],
                CTableProductionDataReuse.batch_number == str_batchno,
            ]
            obj_result = (
            ...
            if obj_result:
                if obj_dbmgr.update(CTableProductionDataReuse, lst_where,
                                    {"time": n_date,
                                     "count": dict_serialNo["value"],
                                     "comment": str_comment}) != EErrorCode.ERROR_SUCCESS:
                    str_message = 'failed to update production data reuse'
                    CLogger().log(CLogger.LOG_LEVELERROR, '[%s] %s (ref_no:%s, batch_no:%s, serial_no:%s)' % (
                        self.__class__.__name__, str_message, str_ref_no, str_batchno, dict_serialNo["serialNo"]))
            else:
                ...
                if obj_dbmgr.merge(new_reuse) != EErrorCode.ERROR_SUCCESS:
                    n_code = EErrorCode.ERROR_OTHER_ERROR
                    str_message = 'failed to create/update production data output'
                    CLogger().log(CLogger.LOG_LEVELERROR, '[%s] %s' % (self.__class__.__name__, str_message))
    ...
    def __get_warehouse(self, obj_session, str_no):
        obj_data = (
                obj_session.query(CTableShipWarehouse)
                .filter(CTableShipWarehouse.no == str_no)
                .first()
            )
        return obj_data.displayName if obj_data else ""


    def __convent_ref_no_sub(self, obj_batch, str_ref_no_sub, n_refno_subCategory):
        lst_temp = str_ref_no_sub.split('@')

        # 餘廢產
        if 1 < len(lst_temp):
            if n_refno_subCategory in [EProcessOrderCategory.REMAIN, EProcessOrderCategory.WASTE, EProcessOrderCategory.PRODUCT] and obj_batch:
                str_ref_no_sub = obj_batch.ref_no
            else:
                str_ref_no_sub = lst_temp[0] + '_' + lst_temp[1]
        return str_ref_no_sub

    def __count_ref_no_sub(self, str_ref_no_sub):
        n_index = 0
        lst_temp = str_ref_no_sub.split('_')

        if 1 < len(lst_temp):
            n_index = int(lst_temp[1]) - 1
            print("")
        return n_index