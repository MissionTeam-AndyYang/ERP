...
from package.contract.contract import CQuotation
...
class CAPIQuotation(CPrivilegeControl):
    ...
    def get(self, str_timezone='', str_id=''):
        ...
        try:
            ...
            n_type = int(request.args.get('type', 0))
            dict_where = self.__fill_query_params()
            n_total, lst_result = CQuotation(n_type).get(n_start, n_count, dict_where)
